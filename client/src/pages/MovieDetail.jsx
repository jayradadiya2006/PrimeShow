...
export const MovieDetail = ({ movieId, onOpenSeatPicker, onBookTickets, onBackToMovies }) => {
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] text-slate-900 pb-20 font-sans">
      ...
      {/* 1. Prominent Top Sub-Header Bar */}
      <div className="bg-[#DBCEA5] border-b border-[#c5ba92] px-4 sm:px-8 py-3 sticky top-[56px] sm:top-[64px] z-30 flex items-center justify-between gap-3 shadow-md text-slate-900">
        <div className="flex items-center gap-3">
          <button
            onClick={handleBackClick}
            className="p-2.5 rounded-xl bg-white hover:bg-[#D90000] hover:text-white text-slate-900 border border-slate-300 transition-all cursor-pointer flex items-center gap-1.5 text-xs font-bold shrink-0 shadow-xs"
          ...
          <h2 className="text-sm sm:text-lg font-bold font-sans text-slate-900 truncate max-w-[200px] sm:max-w-md">
            {movie.title}
          </h2>
        </div>
        ...
        <button
          ...
          className="px-3.5 py-1.5 rounded-full bg-[#D90000] text-white font-extrabold text-xs shadow-md hover:bg-[#b00000] transition-all flex items-center gap-1 cursor-pointer shrink-0"
        >
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4 sm:gap-6">
            ...
            <div className="flex flex-col items-start gap-2 shrink-0">
              ...
              {movie.trailerUrl && (
                <button
                  ...
                  className="w-full py-2 px-3 rounded-xl bg-[#D90000] hover:bg-[#b00000] text-white text-[11px] font-extrabold flex items-center justify-center gap-1.5 shadow-lg shadow-red-600/30 cursor-pointer"
            ...
            <div className="space-y-2 sm:space-y-3 flex-1 text-left">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="px-2.5 py-0.5 rounded-full bg-[#DBCEA5] text-slate-900 border border-[#c5ba92] text-[10px] sm:text-xs font-bold uppercase">
                  {movie.parentalRating || 'UA'}
                </span>
                <span className="px-2.5 py-0.5 rounded-full bg-[#D90000] text-white border border-[#D90000] text-[10px] sm:text-xs font-bold uppercase">
                  {movie.status || 'Now Showing'}
                </span>
                <span className="text-[11px] sm:text-xs text-slate-300">Release: {movie.releaseDate || '2026'}</span>
              </div>
              ...
              {movie.tagline && (
                <p className="text-xs sm:text-sm text-[#DBCEA5] font-medium italic">{movie.tagline}</p>
              )}

              {/* Ratings & Duration */}
              <div className="flex items-center gap-2 sm:gap-4 text-xs font-semibold text-slate-200">
                <span className="flex items-center gap-1 text-amber-400">
                  <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-400" />
                  <strong className="text-xs sm:text-sm text-white">{movie.rating || 9.2}</strong> ({((movie.votesCount || 25000)/1000).toFixed(1)}k votes)
                </span>
                <span>•</span>
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none no-scrollbar [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden pt-1">
                ...
                {Array.isArray(movie.formats) && movie.formats.map((f, i) => (
                  <span key={i} className="px-2.5 py-1 rounded-lg bg-[#DBCEA5] text-slate-900 text-[10px] sm:text-xs font-bold border border-[#c5ba92] shrink-0">
                    {f}
                  </span>
                ))}
              </div>
      ...
      {/* Main Details Body */}
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-6 sm:pt-10 space-y-8 sm:space-y-12 text-slate-900">
        
        {/* 3. About Movie Section */}
        <div className="bg-[#DBCEA5] p-5 sm:p-8 rounded-3xl border border-[#c5ba92] space-y-4 shadow-md">
          <h3 className="text-base sm:text-xl font-bold font-sans text-slate-900 flex items-center gap-2">
            <Film className="w-4 h-4 sm:w-5 sm:h-5 text-[#D90000]" />
            <span>About The Movie</span>
          </h3>
          
          <div className="text-xs sm:text-sm text-slate-800 leading-relaxed max-w-4xl">
            <p>
              ...
              {isLongSynopsis && (
                <button
                  ...
                  className="text-[#D90000] font-bold text-xs cursor-pointer ml-2 hover:underline inline-flex items-center gap-0.5"
                >
          ...
          <div className="pt-3 border-t border-[#c5ba92] flex flex-wrap gap-4 text-xs text-slate-800">
            <div><span className="text-[#D90000] font-bold">Director:</span> {movie.director || 'James Cameron'}</div>
            {movie.producer && <div><span className="text-[#D90000] font-bold">Producer:</span> {movie.producer}</div>}
            {movie.language && <div><span className="text-[#D90000] font-bold">Language:</span> {movie.language}</div>}
          </div>
        </div>

        {/* 4. Conditional Cast & Crew Section */}
        {validCast.length > 0 && (
        ...
        <div id="showtimes-section" className="space-y-6 pt-2">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#c5ba92] pb-4">
            <div>
              <h3 className="text-lg sm:text-2xl font-bold font-sans text-slate-900">Select Showtimes & Theatre</h3>
              <p className="text-xs text-[#D90000] font-semibold">Showing available dates & theatres configured by Admin</p>
            </div>
                ...
                <button
                  ...
                  className={`px-3.5 py-1.5 sm:px-4 sm:py-2 rounded-2xl text-xs font-bold transition-all cursor-pointer whitespace-nowrap ${
                    currentDateSelection === d.date
                      ...
                      : 'bg-[#DBCEA5] text-slate-900 hover:bg-[#caba8e] border border-[#c5ba92]'
                  }`}
                >
          ...
          <div className="space-y-4">
            {isScheduleLoading ? (
              <div className="p-8 text-center bg-[#DBCEA5] rounded-3xl text-slate-900 text-xs font-bold animate-pulse flex items-center justify-center gap-2 border border-[#c5ba92]">
                <Clock className="w-4 h-4 animate-spin text-[#D90000]" />
                <span>Fetching live showtime schedules...</span>
              </div>
            ) : filteredTheatres.length > 0 ? (
              filteredTheatres.map((theatre) => (
                <div key={theatre.id} className="bg-[#DBCEA5] p-4 sm:p-6 rounded-3xl border border-[#c5ba92] space-y-4 shadow-md text-slate-900">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#c5ba92] pb-3">
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <h4 className="text-base sm:text-lg font-bold text-slate-900 flex items-center gap-2">
                          <MapPin className="w-4 h-4 text-[#D90000] shrink-0" />
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 rounded-xl bg-[#D90000] text-white text-[11px] font-bold transition-all flex items-center gap-1 cursor-pointer shadow-xs"
                      ...
                      <p className="text-xs text-slate-700">{theatre.address}</p>
                    </div>

                    <div className="flex flex-wrap items-center gap-1.5">
                      {theatre.facilities?.map((fac, idx) => (
                        <span key={idx} className="px-2.5 py-0.5 rounded-md bg-white text-[10px] text-slate-900 border border-slate-300 font-semibold">
                          {fac}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2.5 sm:gap-3 pt-1">
                    {theatre.shows?.map((show) => {
                      ...
                      if (isUnavailable) {
                        return (
                          <div key={show.id} className="px-4 py-2.5 sm:px-5 sm:py-3 rounded-2xl bg-white/50 text-slate-400 border border-slate-300 text-center font-bold opacity-60 cursor-not-allowed">
                            <div className="text-xs sm:text-sm font-extrabold">{show.time}</div>
                            <div className="text-[9px] sm:text-[10px] text-red-500 font-medium">Slot Offline ({currentDateSelection})</div>
                          </div>
                        );
                      }
                      ...
                      return (
                        <button
                          ...
                          className="px-4 py-2.5 sm:px-5 sm:py-3 rounded-2xl bg-white hover:bg-[#D90000] hover:text-white text-slate-900 border border-slate-300 font-bold transition-all text-center group cursor-pointer shadow-md hover:scale-105"
                        >
                          <div className="text-xs sm:text-sm font-extrabold group-hover:text-white">{show.time}</div>
                          <div className="text-[9px] sm:text-[10px] text-slate-600 group-hover:text-white font-medium">
              ...
              <div className="p-6 sm:p-8 rounded-3xl bg-[#DBCEA5] border border-[#c5ba92] text-center space-y-3 text-slate-900 shadow-md">
                <AlertCircle className="w-8 h-8 text-[#D90000] mx-auto" />
                <h4 className="text-base font-bold text-slate-900">No Theatre Schedules Found in {activeCityToUse} for {currentDateSelection}</h4>
                ...
                {activeCityToUse !== 'All' && (
                  <div className="pt-1 flex justify-center">
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-[#D90000] hover:bg-[#b00000] text-white font-extrabold text-xs cursor-pointer shadow-md"
                    >
                      View All Cities
                    </button>
                  </div>
        ...
        <div className="bg-[#DBCEA5] p-5 sm:p-8 rounded-3xl border border-[#c5ba92] space-y-5 shadow-md text-slate-900">
          <h3 className="text-base sm:text-xl font-bold font-sans text-slate-900 flex items-center gap-2">
            <MessageSquare className="w-4 h-4 sm:w-5 sm:h-5 text-[#D90000]" />
            <span>Audience Reviews & Ratings</span>
          </h3>

          {/* Add Review Form */}
          <form onSubmit={handleReviewSubmit} className="space-y-3 p-3.5 sm:p-4 rounded-2xl bg-white border border-slate-300 shadow-inner">
            <h4 className="text-xs font-bold text-[#D90000] uppercase">Write Your Review</h4>
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-700 font-semibold">Your Rating:</span>
              <div className="flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  ...
                  >
                    <Star className={`w-4 h-4 sm:w-5 sm:h-5 ${star <= userRating ? 'fill-amber-400 text-amber-400' : 'text-slate-300'}`} />
                  </button>
                ))}
              </div>
            </div>
            ...
            <textarea
              ...
              className="w-full p-2.5 rounded-xl bg-white text-slate-900 placeholder-slate-500 border border-slate-300 text-xs font-semibold focus:outline-none"
            ></textarea>

            <div className="flex items-center justify-between">
              <button
                type="submit"
                className="px-5 py-2 rounded-xl bg-[#D90000] hover:bg-[#b00000] text-white font-bold text-xs shadow-md cursor-pointer"
              >
                Post Review
              </button>
              {reviewSubmitted && (
                <span className="text-xs text-[#66DD6A] font-bold">Posted!</span>
              )}
            </div>
          </form>
          ...
          <div className="space-y-3 pt-1">
            {reviewsList.map((rev) => {
              ...
              return (
                <div key={rev.id} className="p-3.5 sm:p-4 rounded-2xl bg-white border border-slate-300 space-y-1.5 shadow-xs">
                  <div className="flex items-center justify-between">
                  ...
                  <p className="text-xs text-slate-800 leading-relaxed">
                    ...
                    {isLongComment && (
                      <button
                        ...
                        className="text-[#D90000] font-bold text-[11px] cursor-pointer ml-1.5 hover:underline inline-block"
                      >
                  ...
                  <span className="text-[10px] text-slate-500 block">{rev.createdAt}</span>
                </div>
              );
            })}
          </div>
      ...
      <footer className="mt-16 border-t border-slate-200 bg-black text-white py-8 text-center text-xs space-y-3">
        <div className="flex items-center justify-center gap-2 text-white font-bold text-sm">
          <span>PrimeShow Cinema</span>
        </div>
        <div className="flex flex-wrap justify-center gap-4 text-[11px] text-slate-300">
          <button onClick={handleBackClick} className="hover:text-[#D90000] cursor-pointer">Home</button>
          <span>•</span>
          <button onClick={handleScrollToShowtimes} className="hover:text-[#D90000] cursor-pointer">Select Showtimes</button>
          <span>•</span>
          <span className="hover:text-[#D90000] cursor-pointer">Privacy Policy</span>
          <span>•</span>
          <span className="hover:text-[#D90000] cursor-pointer">Terms & Conditions</span>
        </div>
        <p className="text-[10px] text-slate-400">© 2026 PrimeShow Entertainment Ltd. All rights reserved.</p>
      </footer>
