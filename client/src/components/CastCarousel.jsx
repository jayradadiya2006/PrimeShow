  ...
  return (
    <div className="w-full my-4">
      ...
      <div className="flex items-center gap-2.5 sm:gap-4 overflow-x-auto pb-3 scrollbar-none">
        ...
        {/* Cast Members */}
        {cast.map((actor, idx) => (
          <div key={idx} className="shrink-0 w-24 sm:w-32 glass-panel rounded-2xl p-2 sm:p-2.5 text-center border border-white/10 hover:border-amber-400/40 transition-colors">
            <div className="w-14 h-14 sm:w-20 sm:h-20 mx-auto rounded-full overflow-hidden mb-1.5 border border-white/20 shadow-md">
              <img
                ...
                alt={actor.name || `Cast ${idx+1}`}
                className="w-full h-full object-cover"
              />
            </div>
            <div className="text-[11px] sm:text-xs font-bold text-white truncate">{actor.name || (typeof actor === 'string' ? actor : 'Artist')}</div>
            {actor.role && (
              <div className="text-[9px] sm:text-[10px] text-amber-400/80 truncate">as {actor.role}</div>
            )}
          </div>
        ))}
      </div>
